...
from layers.UnitaryMatrices.svd import convolution_orthogonal_generator_unitaries

class SymSVD(StreamlinedModule, LipschitzModuleL2):
    def __init__(
        self,
        in_channels,
        out_channels,
    ...
    def construct_U_matrix(self, U0, U1):
        ...
        for k in range(self.num_kernels):
            ...
            U[k, :, :]    = torch.cat((x,y),dim=1).t() / 2.0
        u0, u1 = U0[-1, :, :], U1[-1, :, :]
        x, y = torch.cat((u0, u0, u1, -u1), dim=1).t().chunk(2)
        U[-1, :, :]    = torch.cat((x,y),dim=1).t() / np.sqrt(2.0)
        u0, u1 = U0[0, :, :], U1[0, :, :]
        x, y = torch.cat((u0, u1, u0, -u1), dim=1).t().chunk(2)
        U[0, :, :] = torch.block_diag(u0, u1)
        return U
    ...
    def _gen_weights(self):
        ...
        if not self.streamline or self.weight is None:
            ...
            self.weight = convolution_orthogonal_generator_unitaries(
                self.kernel_size, self.in_channels, self.out_channels, H, UV, self.mask, p_init={(0,0): self.perm_matrix} if self.first else None
            )
