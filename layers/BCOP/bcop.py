...
def block_orth(p1, p2, order=0):
    ...
    if order == 0:
        ...
        kernel2x2[0, 0] = p1.mm(p2)
        kernel2x2[0, 1] = (eye - p1).mm(p2)
        kernel2x2[1, 0] = p1.mm(eye - p2)
        kernel2x2[1, 1] = (eye - p1).mm(eye - p2)
    elif order < 1 and order > 0:
        return block_orth(p1, p2, 1) * order + block_orth(p1, p2, 0) * (1 - order)
    else:
        raise ValueError('order should be 0 or 1, or a number in between')
...
def convolution_orthogonal_generator_projs(ksize, cin, cout, ortho, sym_projs, p_init=None, order=None):
    flipped = False
    if cin > cout:
        flipped = True
        cin, cout = cout, cin
    ...
    p = block_orth(sym_projs[1], sym_projs[0], order[0] if order is not None else 0)

    if p_init is not None:
        p = matrix_conv(p, p_init)
    for _ in range(1, ksize - 1):
        p = matrix_conv(block_orth(sym_projs[_ * 2 + 1], sym_projs[_ * 2], order[_] if order is not None else 0), p)
